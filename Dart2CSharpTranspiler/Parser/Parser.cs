...
// https://github.com/dart-lang/sdk/blob/master/pkg/analyzer/lib/src/generated/parser.dart
...
namespace Dart2CSharpTranspiler.Parser
{
    ...
    public class CommentAndMetadata
    {
         ...
         */
        public readonly List<Annotation> metadata;
    ...
    public class FinalConstVarOrType
    {
         ...
         */
        FinalConstVarOrType(Token keyword, TypeAnnotation type)
        {
    ...
    public class Parser
    {
        public static String ASYNC = Keyword.ASYNC.lexeme;
        ...
        public static String SYNC = Keyword.SYNC.lexeme;

        static String _YIELD = Keyword.YIELD.lexeme;

        public static int _MAX_TREE_DEPTH = 300;

        /**
         * A flag indicating whether the analyzer [Parser] factory method
         * will return a fasta based parser or an analyzer based parser.
         */
        public static bool useFasta = new bool.fromEnvironment("useFastaParser", defaultValue: true);
         ...
         * during the parse.
         */
        public readonly AnalysisErrorListener _errorListener;
         ...
         */
        [Obsolete]
        public bool parseGenericMethods = false;

        public bool allowNativeClause;
         ...
         */
        [Obsolete]
        public bool enableAssertInitializer => true;
         ...
         */
        [Obsolete]
        public void setEnableUriInPartOf(bool enable) { }
         ...
         */
        public bool hasReturnTypeInTypeAlias()
        {
         ...
         */
        [Obsolete]
        public void parseAsync(bool parseAsync) { }

        [Obsolete]
        public bool parseConditionalDirectives => true;

        [Obsolete]
        public void setParseConditionalDirectives(bool value) { }
         ...
         */
        public String computeStringValue(String lexeme, bool isFirst, bool isLast)
        {
         ...
         */
        public SimpleIdentifier createSyntheticIdentifier(bool isDeclaration = false)
        {
         ...
         */
        public SimpleStringLiteral createSyntheticStringLiteral() => astFactory
         ...
         * function expression.
         */
        public bool isFunctionExpression(Token token)
        {
            ...
            if (afterParameters.matchesAny(
                new List<TokenType> { TokenType.OPEN_CURLY_BRACKET, TokenType.FUNCTION }))
            {
                return true;
            }
         ...
         */
        public bool isInitializedVariableDeclaration()
        {
            Keyword keyword = _currentToken.keyword;
            ...
            if (keyword == Keyword.CONST)
            {
                ...
                return !_peek().matchesAny(new List<TokenType>{
                                            TokenType.LT,
        ...
        public ArgumentList parseArgumentList()
        {
            ...
            try
            {
                ...
                List<Expression> arguments = new List<Expression> { argument };
         ...
         *       | identifier
         */
        public Expression parseAssignableExpression(bool primaryAllowed)
        {
            //
         ...
         *       | '?.' identifier
         */
        public Expression parseAssignableSelector(Expression prefix, bool optional,
         ...
         */
        public Expression parseBitwiseAndExpression()
        {
            Expression expression;
            if (_currentToken.keyword == Keyword.SUPER &&
         ...
         */
        public Expression parseBitwiseOrExpression()
        {
            Expression expression;
            if (_currentToken.keyword == Keyword.SUPER &&
         ...
         */
        public Expression parseBitwiseXorExpression()
        {
            Expression expression;
            if (_currentToken.keyword == Keyword.SUPER &&
         ...
         */
        public Expression parseCascadeSection()
        {
        ...
        /**
         * Parse a public class declaration. The [commentAndMetadata] is the metadata to be
         ...
         * public class declaration that was parsed.
         *
         ...
         */
        public CompilationUnitMember parseClassDeclaration(
              CommentAndMetadata commentAndMetadata, Token abstractKeyword)
        {
            ...
            //
            // Check to see whether this might be a public class type alias rather than a class
            // declaration.
            //
            if (type == TokenType.EQ)
            {
        ...
        /**
         * Parse a public class member. The [className] is the name of the public class containing
         * the member being parsed. Return the public class member that was parsed, or `null`
         * if what was found was not a valid public class member.
         *
         *     classMemberDefinition ::=
         *         declaration ';'
         *       | methodSignature functionBody
         */
        public ClassMember parseClassMember(String className)
        {
                ...
                _atGenericFunctionTypeAfterReturnType(_currentToken))
            {
                  ...
                  (_isOperator(next) || next.type == TokenType.EQ_EQ_EQ))
                {
                    ...
                    return _parseOperatorAfterKeyword(commentAndMetadata,
                      ...
                      _peek().matchesAny(new List<TokenType> {
            ...
            Token next1 = _peek();
            bool isFollowedByIdentifier = _tokenMatchesIdentifier(next1);
            if (keyword == Keyword.GET && isFollowedByIdentifier)
            {
                _validateModifiersForGetterOrSetterOrMethod(modifiers);
                return parseGetter(commentAndMetadata, modifiers.externalKeyword,
            ...
            else if (keyword == Keyword.OPERATOR && _isOperator(next1))
            {
                _validateModifiersForOperator(modifiers);
            ...
            else if (!_matchesIdentifier())
            {
                ...
                Token keyword1 = modifiers.varKeyword ??
                ...
                if (keyword1 != null)
                {
                    ...
                    List<VariableDeclaration> variables = new List<VariableDeclaration> { variable };
            ...
            else if (_tokenMatches(next1, TokenType.PERIOD) &&
            ...
            else if (_tokenMatches(next1, TokenType.OPEN_PAREN))
            {
            ...
            else if (next1.matchesAny(new List<TokenType>{
                                        TokenType.EQ,
                                  TokenType.COMMA,
                                  TokenType.SEMICOLON
                                        }))
            ...
            keyword = _currentToken.keyword;
            next1 = _peek();
            isFollowedByIdentifier = _tokenMatchesIdentifier(next1);
            if (keyword == Keyword.GET && isFollowedByIdentifier)
            {
                _validateModifiersForGetterOrSetterOrMethod(modifiers);
                return parseGetter(commentAndMetadata, modifiers.externalKeyword,
            ...
            else if (_tokenMatches(next1, TokenType.LT))
            {
                return _parseMethodDeclarationAfterReturnType(commentAndMetadata,
                    modifiers.externalKeyword, modifiers.staticKeyword, type);
            }
            else if (_tokenMatches(next1, TokenType.OPEN_CURLY_BRACKET))
            {
         ...
         *       | 'hide' identifier (',' identifier)*
         */
        public List<Combinator> parseCombinators()
        {
         ...
         *         annotation*
         */
        public CommentAndMetadata parseCommentAndMetadata()
        {
         ...
         */
        public CommentReference parseCommentReference(
         ...
         *          '(' stringLiteral ')'
         */
        public List<CommentReference> parseCommentReferences(
              List<DocumentationCommentToken> tokens)
        {
            List<CommentReference> references = new List<CommentReference> { };
            bool isInGitHubCodeBlock = false;
            foreach (DocumentationCommentToken token in tokens)
            {
         ...
         *       | topLevelDeclaration
         */
        public CompilationUnit parseCompilationUnit2()
        {
            ...
            if (partOfDirectiveFound && directives.Count > 1)
            {
                ...
                int directiveCount = directives.Count;
                for (int i = 0; i < directiveCount; i++)
                {
                    Directive directive = directives[i];
                    if (directive is PartOfDirective)
         ...
         *       | (public readonly | const) type? staticFinalDeclarationList ';'
         *       | variableDeclaration ';'
         */
        public CompilationUnitMember parseCompilationUnitMember(
              CommentAndMetadata commentAndMetadata)
        {
            ...
            else if (keyword == Keyword.ENUM)
            {
              ...
              _atGenericFunctionTypeAfterReturnType(_currentToken))
            {
                TypeAnnotation returnType1;
                if (keyword == Keyword.VOID)
                {
                    if (_atGenericFunctionTypeAfterReturnType(next))
                    {
                        returnType1 = parseTypeAnnotation(false);
                    }
                    else
                    {
                        returnType1 = astFactory.typeName(
                            astFactory.simpleIdentifier(getAndAdvance()), null);
                    }
                }
                else
                    ...
                    _tokenMatchesIdentifier(next))
                {
                    ...
                    return parseFunctionDeclaration(
                        commentAndMetadata, modifiers.externalKeyword, returnType1);
                }
                else if (keyword == Keyword.OPERATOR && _isOperator(next))
                {
                  ...
                  next.matchesAny(new List<TokenType>{
                                TokenType.OPEN_PAREN,
                        TokenType.OPEN_CURLY_BRACKET,
                        TokenType.FUNCTION,
                        TokenType.LT
                            ...
                            }))
                {
                    return astFactory.topLevelVariableDeclaration(
                        ...
                        parseVariableDeclarationListAfterType(null,
                            _validateModifiersForTopLevelVariable(modifiers), returnType1),
            ...
            else if (!_matchesIdentifier())
            {
                ...
                if (keyword1 == null)
                {
                    keyword1 = modifiers.finalKeyword;
                }
                if (keyword1 == null)
                {
                    keyword1 = modifiers.constKeyword;
                }
                if (keyword1 != null)
                {
                    //
            ...
            {
                TypeName returnType2 = null;
                _validateModifiersForTopLevelFunction(modifiers);
                return parseFunctionDeclaration(
                    commentAndMetadata, modifiers.externalKeyword, returnType2);
            }
            else if (next.matchesAny(new List<TokenType>{
                                    TokenType.EQ,
                              TokenType.COMMA,
                              TokenType.SEMICOLON
                                    }))
         ...
         *       | mapLiteral
         */
        public Expression parseConstExpression()
        {
            Token keyword = getAndAdvance();
            TokenType type = _currentToken.type;
            if (type == TokenType.LT)
         ...
         */
        public ConstructorFieldInitializer parseConstructorFieldInitializer(bool hasThis)
        {
         ...
         *         type ('.' identifier)?
         */
        public ConstructorName parseConstructorName()
        {
         ...
         *       | partDirective
         */
        public Directive parseDirective(CommentAndMetadata commentAndMetadata)
        {
         ...
         *         scriptTag? directive*
         */
        public CompilationUnit parseDirectives2()
        {
            Token firstToken = _currentToken;
            ScriptTag scriptTag = null;
            if (_matches(TokenType.SCRIPT_TAG))
         ...
         *       | singleLineComment*
         */
        public Comment parseDocumentationComment(List<DocumentationCommentToken> tokens)
        {
            if (tokens == null)
            {
                return null;
         ...
         *       | singleLineComment*
         */
        public List<DocumentationCommentToken> parseDocumentationCommentTokens()
        {
            List<DocumentationCommentToken> tokens = new List<DocumentationCommentToken> { };
         ...
         *         identifier ('.' identifier)*
         */
        public DottedName parseDottedName()
        {
         ...
         *         ';'
         */
        public Statement parseEmptyStatement() => astFactory.emptyStatement(getAndAdvance());
         ...
         */
        public EnumDeclaration parseEnumDeclaration(CommentAndMetadata commentAndMetadata)
        {
            Token keyword = getAndAdvance();
            SimpleIdentifier name = parseSimpleIdentifier(isDeclaration: true);
            Token leftBracket = null;
         ...
         */
        public ExportDirective parseExportDirective(CommentAndMetadata commentAndMetadata)
        {
        ...
        {
            List<Expression> expressions = new List<Expression> { parseExpression2() };
            while (_optional(TokenType.COMMA))
            {
         ...
         */
        public Expression parseExpressionWithoutCascade()
        {
        ...
        /**
         * Parse a public class extends clause. Return the public class extends clause that was
         * parsed.
         *
         ...
         *         'extends' type
         */
        public ExtendsClause parseExtendsClause()
        {
            Token keyword = getAndAdvance();
         ...
         *       | type
         */
        public FinalConstVarOrType parseFinalConstVarOrType(bool optional,
         ...
         *         normalFormalParameter (':' expression)?
         */
        public FormalParameter parseFormalParameter(ParameterKind kind,
               bool inFunctionType = false)
        {
         ...
         */
        public FormalParameterList parseFormalParameterList(bool inFunctionType = false)
        {
            if (_matches(TokenType.OPEN_PAREN))
            {
                ...
                if (!_matches(TokenType.SEMICOLON))
                {
                            ...
                            _tokenMatches(_peek(), TokenType.COLON)))
                    {
                        ...
                        variableList = astFactory.variableDeclarationList(
                            ...
                            commentAndMetadata.metadata,
                            null,
                            null, new List<VariableDeclaration> {
                    ...
                    if (_matchesKeyword(Keyword.IN) || type == TokenType.COLON)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                _reportErrorForCurrentToken(
                                    ParserErrorCode.MULTIPLE_VARIABLES_IN_FOR_EACH, [variables.Length.toString()]);
                            }
         ...
         *       | block
         */
        public FunctionBody parseFunctionBody(
         ...
         *         functionSignature functionBody
         */
        public Statement parseFunctionDeclarationStatement()
        {
            Modifiers modifiers = parseModifiers();
         ...
         *         type identifier
         */
        public GenericFunctionType parseGenericFunctionTypeAfterReturnType(
              TypeAnnotation returnType)
        {
         ...
         */
        public GenericTypeAlias parseGenericTypeAlias(
         ...
         */
        public MethodDeclaration parseGetter(CommentAndMetadata commentAndMetadata,
         ...
         *         identifier (',' identifier)*
         */
        public List<SimpleIdentifier> parseIdentifierList()
        {
         ...
         */
        public Expression parseIfNullExpression()
        {
         ...
         */
        public ImplementsClause parseImplementsClause()
        {
            Token keyword = getAndAdvance();
         ...
         */
        public ImportDirective parseImportDirective(CommentAndMetadata commentAndMetadata)
        {
         ...
         *         identifier ('=' expression)?
         */
        public FieldDeclaration parseInitializedIdentifierList(
              CommentAndMetadata commentAndMetadata,
              Token staticKeyword,
         ...
         */
        public InstanceCreationExpression parseInstanceCreationExpression(Token keyword)
        {
         ...
         *         identifier ':'
         */
        public Label parseLabel(bool isDeclaration = false)
        {
         ...
         */
        public LibraryDirective parseLibraryDirective(
              CommentAndMetadata commentAndMetadata)
        {
            Token keyword = getAndAdvance();
         ...
         *         identifier ('.' identifier)*
         */
        public LibraryIdentifier parseLibraryIdentifier()
        {
            List<SimpleIdentifier> components = new List<SimpleIdentifier> { };
            components.Add(parseSimpleIdentifier());
            while (_optional(TokenType.PERIOD))
         ...
         */
        public ListLiteral parseListLiteral(Token modifier, TypeArgumentList typeArguments)
        {
            ...
            try
            {
                List<Expression> elements = new List<Expression> { parseExpression2() };
                while (_optional(TokenType.COMMA))
                {
                    if (_matches(TokenType.CLOSE_SQUARE_BRACKET))
                    {
         ...
         *       | mapLiteral
         */
        public TypedLiteral parseListOrMapLiteral(Token modifier)
        {
            TypeArgumentList typeArguments = _parseOptionalTypeArguments();
            if (_matches(TokenType.OPEN_CURLY_BRACKET))
            {
         ...
         */
        public MapLiteral parseMapLiteral(Token modifier, TypeArgumentList typeArguments)
        {
            Token leftBracket = getAndAdvance();
            if (_matches(TokenType.CLOSE_CURLY_BRACKET))
            {
            ...
            try
            {
                List<MapLiteralEntry> entries = new List<MapLiteralEntry> { parseMapLiteralEntry() };
                while (_optional(TokenType.COMMA))
                {
                    if (_matches(TokenType.CLOSE_CURLY_BRACKET))
                    {
         ...
         *         expression ':' expression
         */
        public MapLiteralEntry parseMapLiteralEntry()
        {
         ...
         */
        public Modifiers parseModifiers()
        {
         ...
         *         instanceCreationExpression
         */
        public InstanceCreationExpression parseNewExpression() =>
         ...
         *       | functionSignature functionBody
         */
        public Statement parseNonLabeledStatement()
        {
            ...
            else if (type.isKeyword && !_currentToken.keyword.isBuiltInOrPseudo)
            {
                ...
                {
                    Token next = _currentToken.next;
                    if (next.matchesAny(new List<TokenType>{
                                          TokenType.LT,
                                          TokenType.OPEN_CURLY_BRACKET,
                                          TokenType.OPEN_SQUARE_BRACKET,
                                          TokenType.INDEX
         ...
         *       | metadata identifier
         */
        public NormalFormalParameter parseNormalFormalParameter(
             bool inFunctionType = false)
        {
            Token covariantKeyword;
            CommentAndMetadata commentAndMetadata = parseCommentAndMetadata();
         ...
         */
        public MethodDeclaration parseOperator(CommentAndMetadata commentAndMetadata,
         ...
         */
        public Directive parsePartOrPartOfDirective(CommentAndMetadata commentAndMetadata)
        {
            if (_tokenMatchesKeyword(_peek(), Keyword.OF))
            {
                return _parsePartOfDirective(commentAndMetadata);
         ...
         *         identifier ('.' identifier)?
         */
        public Identifier parsePrefixedIdentifier()
        {
         ...
         */
        public RedirectingConstructorInvocation parseRedirectingConstructorInvocation(
              bool hasPeriod)
        {
            Token keyword = getAndAdvance();
            Token period = null;
         ...
         *         'rethrow'
         */
        public Expression parseRethrowExpression() =>
         ...
         */
        public MethodDeclaration parseSetter(CommentAndMetadata commentAndMetadata,
              Token externalKeyword, Token staticKeyword, TypeAnnotation returnType)
        {
            Token propertyKeyword = getAndAdvance();
            SimpleIdentifier name = parseSimpleIdentifier(isDeclaration: true);
         ...
         *         IDENTIFIER
         */
        public SimpleIdentifier parseSimpleIdentifier(
         ...
         */
        public Statement parseStatement2()
        {
            if (_treeDepth > _MAX_TREE_DEPTH)
            {
                throw new _TooDeepTreeError();
         ...
         */
        public SuperConstructorInvocation parseSuperConstructorInvocation()
        {
            Token keyword = getAndAdvance();
            Token period = null;
            SimpleIdentifier constructorName = null;
         ...
         *         '#' identifier ('.' identifier)*
         */
        public SymbolLiteral parseSymbolLiteral()
        {
         ...
         *         'throw' expression
         */
        public Expression parseThrowExpression()
        {
            Token keyword = getAndAdvance();
            TokenType type = _currentToken.type;
         ...
         */
        public Expression parseThrowExpressionWithoutCascade()
        {
            Token keyword = getAndAdvance();
            TokenType type = _currentToken.type;
            if (type == TokenType.SEMICOLON || type == TokenType.CLOSE_PAREN)
         ...
         *         'finally' block
         */
        public Statement parseTryStatement()
        {
         ...
         *         returnType? name
         */
        public TypeAlias parseTypeAlias(CommentAndMetadata commentAndMetadata)
        {
            Token keyword = getAndAdvance();
            if (_matchesIdentifier())
            {
         ...
         *       | functionType
         */
        public TypeAnnotation parseTypeAnnotation(bool inExpression)
        {
            TypeAnnotation type = null;
         ...
         *         type (',' type)*
         */
        public TypeArgumentList parseTypeArgumentList()
        {
            Token leftBracket = getAndAdvance();
            List<TypeAnnotation> arguments = new List<TypeAnnotation>{
        ...
        public TypeName parseTypeName(bool inExpression)
        {
         ...
         *       | typeNotVoidWithoutFunction
         */
        public TypeAnnotation parseTypeNotVoid(bool inExpression)
        {
            TypeAnnotation type = null;
            if (_atGenericFunctionTypeAfterReturnType(_currentToken))
            {
         ...
         */
        public TypeParameterList parseTypeParameterList()
        {
            Token leftBracket = getAndAdvance();
            List<TypeParameter> typeParameters = new List<TypeParameter> { parseTypeParameter() };
            while (_optional(TokenType.COMMA))
            {
         ...
         *       | typeNotVoidWithoutFunction
         */
        public TypeAnnotation parseTypeWithoutFunction(bool inExpression)
        {
        ...
        public VariableDeclaration parseVariableDeclaration()
        {
            ...
            // annotations before variable declarations (e.g. "String [Obsolete] s;").
            ...
            // of a construct like "public class C { int [Obsolete] foo() {} }" (i.e. the
         ...
         */
        public VariableDeclarationList parseVariableDeclarationListAfterMetadata(
              CommentAndMetadata commentAndMetadata)
        {
         ...
         *         finalConstVarOrType variableDeclaration (',' variableDeclaration)*
         */
        public VariableDeclarationList parseVariableDeclarationListAfterType(
              CommentAndMetadata commentAndMetadata,
              Token keyword,
              TypeAnnotation type)
        {
         ...
         *         variableDeclarationList ';'
         */
        public VariableDeclarationStatement parseVariableDeclarationStatementAfterMetadata(
              CommentAndMetadata commentAndMetadata)
        {
         ...
         */
        public Token skipFormalParameterList(Token startToken)
        {
         ...
         */
        public Token skipGenericFunctionTypeAfterReturnType(Token startToken)
        {
         ...
         *         identifier ('.' identifier)?
         */
        public Token skipPrefixedIdentifier(Token startToken)
        {
         ...
         *         IDENTIFIER
         */
        public Token skipSimpleIdentifier(Token startToken)
        {
         ...
         *       | SINGLE_LINE_STRING+
         */
        public Token skipStringLiteral(Token startToken)
        {
         ...
         */
        public Token skipTypeAnnotation(Token startToken)
        {
            Token next = null;
         ...
         *         type (',' type)*
         */
        public Token skipTypeArgumentList(Token startToken)
        {
            Token token = startToken;
         ...
         *         qualified typeArguments?
         */
        public Token skipTypeName(Token startToken)
        {
         ...
         */
        public Token skipTypeParameterList(Token startToken)
        {
         ...
         */
        public Token skipTypeWithoutFunction(Token startToken)
        {
        ...
        bool _couldBeStartOfCompilationUnitMember()
        {
            ...
            {
                // This looks like the start of a public class definition
                return true;
            }
            else if (keyword == Keyword.TYPEDEF &&
              nextType != TokenType.PERIOD &&
        ...
        List<List<int>> _getCodeBlockRanges(String comment)
        {
            ...
            if (StringUtilities.startsWith4(comment, index, 0x20, 0x20, 0x20, 0x20))
            {
                ...
                ranges.Add(new List<int> { index, end });
                index = end;
            }
            while (index < length)
            {
        ...
        /**
         * Parse a list of public class members. The [className] is the name of the class
         ...
         * of public class members that were parsed.
         *
         *     classMembers ::=
        ...
        /**
         * Parse a public class type alias. The [commentAndMetadata] is the metadata to be
         ...
         * public class type alias that was parsed.
         *
         *     classTypeAlias ::=
        ...
        List<Configuration> _parseConfigurations()
        {
            ...
            {
                configurations = configurations == null ? new List<Configuration> { } : configurations;
             ...
             bool inFunctionType = false)
        {
            ...
            //
            leftSquareBracket = leftSquareBracket == null ? leftCurlyBracket : leftSquareBracket;
            rightSquareBracket = rightSquareBracket == null ? rightCurlyBracket : rightSquareBracket;
            ...
            ParserErrorCode missingNameError, Token missingNameToken)
        {
            ...
            return astFactory
                .libraryIdentifier(new List<SimpleIdentifier> { createSyntheticIdentifier() });
        }
        ...
        /**
         * Parse a public class native clause. Return the native clause that was parsed.
         *
        ...
        StringLiteral _parseUri()
        {
            ...
            // top-level keywords?
            bool isKeywordAfterUri(Token token1) =>
                token1.lexeme == Keyword.AS.lexeme ||
                token1.lexeme == _HIDE ||
                token1.lexeme == _SHOW;
            TokenType type1 = _currentToken.type;
            if (type1 != TokenType.STRING &&
                type1 != TokenType.SEMICOLON &&
                !isKeywordAfterUri(_currentToken))
            {
                // Attempt to recover in the case where the URI was not enclosed in
                // quotes.
                Token token = _currentToken;
                bool isValidInUri(Token token2)
                {
            ...
            if (next.matchesAny(new List<TokenType> {
                ...
                (_tokenMatchesIdentifier(next) &&
                    (next.next.matchesAny(
                        new List<TokenType>{TokenType.COMMA, TokenType.CLOSE_PAREN
                            }))))
            {
        ...
        {
            foreach (FormalParameter parameter in parameterList.parameters)
            {
        ...
        /**
         * Validate that the given set of [modifiers] is appropriate for a public class and
    ...
    /**
     * Instances of this public class are thrown when the parser detects that AST has
     ...
     */
    public class _TooDeepTreeError : Exception { }
}