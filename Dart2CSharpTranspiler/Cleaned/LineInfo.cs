...
//https://github.com/dart-lang/sdk/blob/master/pkg/analyzer/lib/source/line_info.dart
...
namespace Dart2CSharpTranspiler.Parser
{
    ...
    public class CharacterLocation : LineInfo_Location
    {
         ...
         */
        public CharacterLocation(int lineNumber, int columnNumber)
    ...
    public class LineInfo
    {
         ...
         */
        public LineInfo(List<int> lineStarts)
        {
            this.lineStarts = lineStarts;
            if (lineStarts == null)
            {
                throw new ArgumentException("lineStarts must be non-null");
            }
            else if (lineStarts.Count < 1)
            {
                throw new ArgumentException("lineStarts must be non-empty");
            }
        }
         ...
         */
        // ignore: deprecated_member_use
        public LineInfo_Location getLocation(int offset)
        {
            var min = 0;
            var max = lineStarts.Count - 1;
            ...
            if (offset >= lineStarts[_previousLine])
            {
                ...
                if (min == lineStarts.Count - 1 || offset < lineStarts[min + 1])
                {
         ...
         */
        public int getOffsetOfLineAfter(int offset)
        {
